...
counter = 0             #Couter
chainage =0             #Last Point Ch
interpolPoint = []           #Last interpolated point
lastpointDist=0          #Distance from last interpolated point to last original point
...
for line in inFile:
    ...
    if lineNo > 1:
        ...
        if curPoint[3] == prvPoint[3]:      
            ...
            # For gaps larger than interval
            while (distFromLastPoint / userInterval) > 1:
                counter += 1
                chainage += userInterval
                distFromLastPoint = distFromLastPoint - userInterval
                
                #print str(distFromLastPoint) +' distance from last point'
                #print str(counter)+ ' counter'
                #print str(remainder) + ' remaider'

                NewPointStr = str(newPoint(prvPoint[:2],curPoint[:2], (userInterval*counter)-remainder,distBet)).rstrip(')').lstrip('(').replace(" ","")
                #print curPoint[3].rstrip('\n')+","+str(round(100,3))+","+NewPointStr+"\n"
                outFile.write(curPoint[3].rstrip('\n')+","+str(round(chainage,3))+","+NewPointStr+"\n")
                
                
                
            remainder = distFromLastPoint
            counter= 0

        else:      #Different Road
            #print prvPoint[3].rstrip('\n')+","+str(round(100,3))+","+prvPoint[0]+","+prvPoint[1]+"\n"
            ...
            print prvPoint[3].rstrip('\n')
            print str(distBet)+ ' distanceBetween'
            print str(lastpointDis)+ ' corrected distancebetween'
            print str(chainage)+' chainage'
            print str(round(chainage+lastpointDis,3))
