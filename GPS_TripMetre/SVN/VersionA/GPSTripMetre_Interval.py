...
def newPoint(pt0,pt1,lineCh):
    ...
    DistPTApart = DistPTApart - lineCh
    print 'Line Ch'
    print DistPTApart
...
for line in inFile:
    ...
    if lineNo > 1:
        ...
        elif NewPointBol == 1: # Sets previous points attributes to newly create point
            print 'new points attributes'
            InterCh = NewPointCh
            NewPointBol = 0
            prvPoint = prvLine.split(',')
            curPoint = line.split(',')
        ...
        if curPoint[3] == prvPoint[3]:       #Same Road
            ...
            if InterCh/userInterval >= 1:
                ...
                InterCh = 0
                NewPointBol = 1  #Set boolean value as true

                #Set new point coordinates to previous points value..
                #prvLine = curPoint[3].rstrip('\n')+","+str(round((PointCh/1000),3))+","+str(newPoint(prvPoint[:2],curPoint[:2],(InterCh - userInterval))).rstrip(')').lstrip('(')+"\n"

                prvLine = str(newPoint(prvPoint[:2],curPoint[:2],(InterCh - userInterval))).rstrip(')').lstrip('(')+","+str(round((PointCh/1000),3))+","+curPoint[3]
